...
import type { FFmpegProgress } from '@/services/FFmpegService';
...
interface VideoMergePanelProps {
  ...
  mergeProgress?: FFmpegProgress | null;
}
...
export const VideoMergePanel = ({
  ...
  useEffect(() => {
    if (mergeProgress && mergeProgress.progress > 0 && !startTimeRef.current) {
      startTimeRef.current = Date.now();
    }
    if (!mergeProgress || mergeProgress.stage === 'complete') {
      startTimeRef.current = null;
    }
  }, [mergeProgress]);

  const getETA = (): string => {
    if (!mergeProgress || !startTimeRef.current || mergeProgress.progress <= 0) return '...';
    const elapsed = (Date.now() - startTimeRef.current) / 1000;
    const progress = mergeProgress.progress / 100;
  ...
  const getPreviewAnimation = (effectId: string) => {
    switch (effectId) {
      ...
      case 'dissolve': return 'animate-fade-in';
      ...
      case 'glitch': return 'animate-pulse';
      default: return '';
    }
  };

  const showProgress = isProcessing && mergeProgress;
  ...
  return (
    ...
    >
      ...
      {/* Progress Overlay */}
      {showProgress && (
        <div className="p-4 border-b border-border shrink-0 space-y-3 bg-primary/5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">{mergeProgress.message}</span>
            </div>
            <span className="text-sm font-bold text-primary">
              %{Math.round(mergeProgress.progress)}
            </span>
          </div>
          <Progress value={mergeProgress.progress} className="h-2" />
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span className="capitalize">{mergeProgress.stage}</span>
            <div className="flex items-center gap-1">
              <Clock className="w-3 h-3" />
              <span>Kalan: {getETA()}</span>
            </div>
          </div>
        </div>
      )}
